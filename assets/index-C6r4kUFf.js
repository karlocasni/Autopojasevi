...
`;document.head.appendChild(ra);function Gi(){const r=document.createElement("section");return r.className="section reviews-slider",r.innerHTML=`
  ...
  `,O.getReviews().then(e=>{if(!e||e.length===0){r.innerHTML=`
    ...
    `;const s=r.querySelector("#reviews-track"),a=r.querySelector("#slider-prev"),n=r.querySelector("#slider-next"),i=r.querySelector("#slider-dots");let o=0;const l=e.length;for(let h=0;h<l;h++){const m=document.createElement("button");m.className=`slider-dot ${h===0?"active":""}`,m.addEventListener("click",()=>c(h)),i.appendChild(m)}const d=()=>{s.querySelectorAll(".review-card").forEach((m,f)=>{m.classList.toggle("active",f===o)}),i.querySelectorAll(".slider-dot").forEach((m,f)=>{m.classList.toggle("active",f===o)})},c=h=>{o=h,d()},u=()=>{o=(o+1)%l,d()},g=()=>{o=(o-1+l)%l,d()};a.addEventListener("click",g),n.addEventListener("click",u);let p=setInterval(u,5e3);r.addEventListener("mouseenter",()=>{clearInterval(p)}),r.addEventListener("mouseleave",()=>{p=setInterval(u,5e3)}),setTimeout(d,0)}),r}const sa=document.createElement("style");sa.textContent=`
  .reviews-slider {
    background: var(--color-secondary);
  }
...
`;document.head.appendChild(ca);function ao({onNext:r,onBack:e,initialData:t={}}){const s=document.createElement("div");s.className="booking-step step-calendar";const a=new Date;let n=a.getMonth(),i=a.getFullYear(),o=t.date||null,l=t.time||null;s.innerHTML=`
  ...
  `;const d=async()=>{const u=["Siječanj","Veljača","Ožujak","Travanj","Svibanj","Lipanj","Srpanj","Kolovoz","Rujan","Listopad","Studeni","Prosinac"];s.querySelector("#calendar-month").textContent=`${u[n]} ${i}`;const g=new Date(i,n,1),h=new Date(i,n+1,0).getDate(),m=g.getDay()===0?6:g.getDay()-1,f=s.querySelector("#calendar-days");f.innerHTML='<div class="calendar-loading" style="grid-column: 1 / -1; text-align: center; padding: 20px; color: var(--color-text-muted);">Učitavanje...</div>';const v=s.querySelector("#prev-month"),y=s.querySelector("#next-month");v&&(v.disabled=!0),y&&(y.disabled=!0);try{const b=await O.getCalendarAvailability(i,n);f.innerHTML="";for(let x=0;x<m;x++){const j=document.createElement("div");j.className="calendar-day empty",f.appendChild(j)}const k=new Date;k.setHours(k.getHours()+24);const w=!1;for(let x=1;x<=h;x++){const j=new Date(i,n,x),_=new Date(j);_.setHours(23,59,59);const T=_<k,B=b[x]||{status:"unavailable",count:0};let F=B.status;const N=document.createElement("button");let Z=F==="unavailable";w&&B.count>0,N.className=`calendar-day ${F} ${T?"past":""}`,N.textContent=x,N.disabled=T||Z,N.disabled||N.addEventListener("click",()=>{o=`${i}-${String(n+1).padStart(2,"0")}-${String(x).padStart(2,"0")}`,f.querySelectorAll(".calendar-day").forEach(K=>K.classList.remove("selected")),N.classList.add("selected"),c(o)}),f.appendChild(N)}}catch(b){console.error("Error rendering calendar:",b),f.innerHTML='<div style="grid-column: 1/-1; color: var(--color-unavailable); text-align: center;">Greška pri učitavanju kalendara.</div>'}finally{v&&(v.disabled=!1),y&&(y.disabled=!1)}},c=async u=>{const g=s.querySelector("#time-slots"),p=s.querySelector("#time-slots-grid"),h=await O.getTimeSlots(u),m=new Date;m.setHours(m.getHours()+24),p.innerHTML=h.map(f=>{const[v,y]=f.time.split(":"),b=new Date(u);b.setHours(parseInt(v),parseInt(y));const k=b<m,w=!f.available||k;return`
...
`;document.head.appendChild(ga);const Oe={async login(r,e){var t,s;try{const{data:a,error:n}=await X.auth.signInWithPassword({email:r,password:e});if(n)throw n;if(!(((s=(t=a.user)==null?void 0:t.user_metadata)==null?void 0:s.role)==="admin"))throw await this.logout(),new Error("Unauthorized: Admin access required");return{user:a.user,session:a.session,error:null}}catch(a){return console.error("Login error:",a),{user:null,session:null,error:a}}},async logout(){try{const{error:r}=await X.auth.signOut();if(r)throw r;return{error:null}}catch(r){return console.error("Logout error:",r),{error:r}}},async resetPassword(r){try{const{error:e}=await X.auth.resetPasswordForEmail(r,{redirectTo:`${window.location.origin}/admin/reset-password`});if(e)throw e;return{error:null}}catch(e){return console.error("Password reset error:",e),{error:e}}},async updatePassword(r){try{const{error:e}=await X.auth.updateUser({password:r});if(e)throw e;return{error:null}}catch(e){return console.error("Update password error:",e),{error:e}}},async getCurrentUser(){try{const{data:{user:r},error:e}=await X.auth.getUser();if(e)throw e;return{user:r,error:null}}catch(r){return console.error("Get user error:",r),{user:null,error:r}}},async isAuthenticated(){var r;try{const{data:{session:e}}=await X.auth.getSession();if(!e)return!1;const{user:t}=await this.getCurrentUser();return((r=t==null?void 0:t.user_metadata)==null?void 0:r.role)==="admin"}catch(e){return console.error("Auth check error:",e),!1}},async createAdmin(r,e){try{const{data:t,error:s}=await X.rpc("create_admin_user",{new_email:r,new_password:e});if(s)throw s;return{user:t,error:null}}catch(t){return console.error("Create admin error:",t),{user:null,error:t}}},async listAdmins(){try{const{data:r,error:e}=await X.rpc("get_admins");if(e)throw e;return{admins:r,error:null}}catch(r){return console.error("List admins error:",r),{admins:[],error:r}}},async deleteAdmin(r){try{const{error:e}=await X.rpc("delete_admin_user",{target_user_id:r});if(e)throw e;return{success:!0,error:null}}catch(e){return console.error("Delete admin error:",e),{success:!1,error:e}}},onAuthStateChange(r){return X.auth.onAuthStateChange(r)}};function co(){const r=document.createElement("div");r.className="page-admin";let e="dashboard";const t=()=>{r.innerHTML="",r.innerHTML=`
    ...
    `}return g.forEach(m=>{m.addEventListener("click",()=>{e=m.dataset.view,h(m.dataset.view)})}),setTimeout(()=>{h("dashboard")},0),r};function s(){const c=document.createElement("div");return c.innerHTML=`
  ...
  `;const g=c.querySelector("#reservations-tbody"),p=c.querySelector("#reservation-modal"),h=c.querySelector("#modal-overlay"),m=c.querySelector("#modal-content"),f=c.querySelector("#modal-actions"),v=c.querySelector("#close-modal-btn"),y=()=>{p.style.display="none",h.style.display="none"};v.addEventListener("click",y),h.addEventListener("click",y);const b=c.querySelectorAll(".filter-pill");b.forEach(j=>{j.addEventListener("click",()=>{u=j.dataset.value,b.forEach(_=>_.classList.remove("active")),j.classList.add("active"),k()})});async function k(){g.innerHTML='<tr><td colspan="6" style="text-align: center; padding: var(--spacing-xl);">Učitavanje...</td></tr>';try{let j=await O.getReservations();if(u!=="all"&&(j=j.filter(_=>_.status===u)),j.length===0){g.innerHTML=`
    ...
    `,c.appendChild(u);const g=u.querySelector("#services-list");return(async()=>{if(g.innerHTML="Učitavanje...",await O.loadServices(),!O.services||O.services.length===0){g.innerHTML="<p>Nema dostupnih usluga.</p>";return}g.innerHTML=`
        ...
        `,g.querySelectorAll(".is-request-price-checkbox").forEach(h=>{h.addEventListener("change",m=>{const f=m.target.dataset.id,v=g.querySelector(`#price-inputs-${f}`);v.style.display=m.target.checked?"none":"block"})}),g.querySelectorAll(".is-from-checkbox").forEach(h=>{h.addEventListener("change",m=>{const f=m.target.dataset.id,v=g.querySelector(`#price_to_container_${f}`);v.style.display=m.target.checked?"flex":"none"})}),g.querySelectorAll(".save-service-btn").forEach(h=>{h.addEventListener("click",async m=>{const f=m.target.dataset.id,v=g.querySelector(`.service-price-input[data-id="${f}"]`),y=g.querySelector(`#is_from_${f}`),b=g.querySelector(`#is_request_price_${f}`),k=g.querySelector(`.service-price-to-input[data-id="${f}"]`),w=b.checked,x=v.value?parseFloat(v.value):0;if(!w&&isNaN(x)){alert("Molimo unesite ispravnu osnovnu cijenu.");return}let j={};if(f==="pojasevi"){const T=g.querySelector(`.service-price-disassembled-input[data-id="${f}"]`);T&&(j.price_disassembled=parseFloat(T.value)||0)}if(f==="zvjezdano-nebo"){const T=g.querySelector(`.service-price-star-input[data-id="${f}"]`),B=g.querySelector(`.service-price-500-input[data-id="${f}"]`),F=g.querySelector(`.service-price-750-input[data-id="${f}"]`);T&&(j.price_per_star=parseFloat(T.value)||0),B&&(j.price_500_stars=parseFloat(B.value)||0),F&&(j.price_750_stars=parseFloat(F.value)||0)}const _=m.target.textContent;m.target.textContent="Spremanje...",m.target.disabled=!0;try{await O.updateServiceConfig(f,{price:x,is_from:y.checked,price_to:k&&k.value?parseFloat(k.value):null,is_request_price:w,...j}),alert("Spremljeno!")}catch(T){console.error(T),alert("Greška pri spremanju.")}finally{m.target.disabled=!1,m.target.textContent=_}})})})(),c}function o(){const c=document.createElement("div");c.innerHTML='<h1 class="admin-title">Recenzije</h1>';const u=document.createElement("div");u.className="settings-card glass";const g=document.createElement("button");g.className="btn btn-primary",g.textContent="Dodaj Recenziju",g.style.marginBottom="var(--spacing-lg)";const p=document.createElement("div");p.className="glass",p.style.cssText="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); width:90%; max-width:500px; padding:var(--spacing-xl); z-index:1000;",p.innerHTML=`
...
`;document.head.appendChild(ma);function ho(){const r=document.createElement("div");r.className="page-terms",r.appendChild(Dt());const e=document.createElement("main");return e.className="terms-main",e.innerHTML=`
  ...
  `,r.appendChild(e),r.appendChild(Ht()),r}const fa=document.createElement("style");fa.textContent=`
    .terms-content h3 {
        color: var(--color-accent);
        margin-top: 2rem;
        margin-bottom: 1rem;
...
`;document.head.appendChild(fa);function po(){const r=document.createElement("div");r.className="page-container not-found-page",r.innerHTML=`
